...
const WP_API_CACHE = '/js/cached-posts.json';
...
function createProgramCard(program) {
    ...
    <div class="rounded-xl border bg-card text-card-foreground overflow-hidden border-none shadow-sm group hover:shadow-lg transition-all duration-300 flex flex-col h-full motion-safe" data-program-id="${program.id}">
        ...
        <div class="flex items-center p-6 pt-0">
            <a href="${program.url}" target="_blank" class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 shadow h-9 px-4 py-2 w-full bg-primary text-white hover:bg-primary/90">了解更多</a>
        </div>
    </div>
    `;
}
